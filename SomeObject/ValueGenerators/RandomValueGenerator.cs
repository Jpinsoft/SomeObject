...
namespace Jpinsoft.SomeObject.ValueGenerators
{
    public class RandomValueGenerator : IValueGenerator
    {
        ...
        public RandomValueGenerator(int minValue, int maxValue, int minCollectionLength, int maxCollectionLength, bool generateUpperCaseString)
        {
            if (minCollectionLength < 0 || minCollectionLength > maxCollectionLength)
                throw new ArgumentException("'MinCollectionLength' must be greater than or equal zero. MaxCollectionLength must be greater than MinCollectionLength. Parameters: minCollectionLength, maxCollectionLength");

            if (maxValue < 0 || minValue > maxValue)
                throw new ArgumentException("'MaxValue' must be greater than or equal zero. MaxValue must be greater than MinValue. Parameters: maxValue, minValue.");
